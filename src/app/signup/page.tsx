...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!data.session) { setSuccess("Check your email for confirmation link"); setIsLoading(false); return; }
    ...
    <div className="flex h-screen w-screen bg-[#0B0F14] text-foreground select-none overflow-hidden">
      <div className="flex-1 flex flex-col justify-center px-8 sm:px-16 md:px-24 bg-[#0B0F14] z-10 border-r border-border/40">
        <div className="mx-auto w-full max-w-sm space-y-7">
          <div className="flex items-center gap-3"><Link href="/" className="flex items-center gap-2.5"><img src="/logo1.png" alt="AgentFlow" className="h-10 w-10 rounded-lg overflow-hidden" /><span className="text-base font-extrabold tracking-tight">AgentFlow</span></Link></div>
          <div className="space-y-1.5"><h2 className="font-h2 font-bold text-foreground">Create Account</h2><p className="text-xs text-muted">Join the next-generation multi-agent orchestra.</p></div>
          {error && <div className="p-3 rounded-lg bg-red-500/10 border border-red-500/30 text-red-400 text-xs">{error}</div>}
          {success && <div className="p-3 rounded-lg bg-green-500/10 border border-green-500/30 text-green-400 text-xs">{success}</div>}
          <div className="grid grid-cols-2 gap-3">
            <Button variant="secondary" size="sm" type="button" className="w-full" onClick={handleGitHubSignup} disabled={isLoading}><GithubIcon className="h-4 w-4 mr-2" />GitHub</Button>
          ...
          <div className="relative flex items-center justify-center"><div className="absolute inset-0 flex items-center"><div className="w-full border-t border-border/80" /></div><span className="relative bg-[#0B0F14] px-3.5 text-[10px] uppercase font-bold text-muted/80">Or sign up with</span></div>
          ...
          <p className="text-xs text-muted text-center">Already have an account? <Link href="/login" className="text-accent hover:underline font-semibold">Sign In instead</Link></p>
        </div>
      </div>
      <div className="hidden lg:flex flex-1 flex-col items-center justify-center p-12 bg-gradient-to-tr from-[#131A23] to-[#0B0F14] relative overflow-hidden">
        ...
        <div className="max-w-md text-left space-y-6 relative z-10">
          ...
          <div className="space-y-4"><h3 className="font-h2 font-bold text-foreground leading-snug">Establish Your Agent Infrastructure First.</h3><p className="text-xs text-muted leading-relaxed">Do not build fragmented code pages. Set up nodes, custom memory modules, RAG schemas, and deployment integrations inside our visual, unified flow dashboard.</p></div>
        </div>
      </div>
    </div>
  );
