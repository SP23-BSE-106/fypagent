...
export const Sidebar: React.FC<SidebarProps> = ({ className }) => {
  ...
  return (
    <motion.aside animate={{ width: isCollapsed ? 70 : 260 }} transition={{ duration: 0.25, ease: "easeInOut" }} className={cn("relative flex flex-col h-screen bg-surface border-r border-border/80 shrink-0 text-foreground", className)}>
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link key={item.name} href={item.href} className={cn("group flex items-center rounded-lg px-3.5 py-2.5 text-xs font-semibold tracking-wide transition-all relative", isActive ? "text-accent bg-accent-muted border border-accent/25" : "text-muted hover:bg-surface-light/40 hover:text-foreground border border-transparent")}>
              <Icon className={cn("h-4.5 w-4.5 shrink-0", isActive ? "text-accent" : "text-muted group-hover:text-foreground")} />
      ...
      <div className="p-3 border-t border-border/40 bg-surface/30">
        <div className="flex items-center gap-3 overflow-hidden">
          <div className="h-8.5 w-8.5 rounded-full bg-accent/20 border border-accent/20 flex items-center justify-center text-xs font-bold text-accent shrink-0">{loading ? "..." : getUserInitials()}</div>
